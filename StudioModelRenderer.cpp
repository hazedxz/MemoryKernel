...
void StudioRenderModel ( void )
{
	...
	if((cvar.espteam || isEnemy(ent->index)) && bIsValidEnt(ent) && cvar.chamsglow && !cvar.rush)
	{
		{
	...
	if((cvar.espteam || isEnemy(ent->index)) && bIsValidEnt(ent) && cvar.chams && !cvar.rush) 
	{
		{
			ent->curstate.rendermode = 0;
			ent->curstate.renderfx = 0;
			...
			if(cvar.chams != 1) glDisable(GL_TEXTURE_2D);
