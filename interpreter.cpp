...
void  CommandInterpreter::Add( const char* name, int type, void* data  )
{
	if(names.find(name))
	{
		Con_Echo("%s is already registered.\n",name);
		return;
	}
...
{	
	...
	for(names.it_start();names.it_running();names.it_next())
	{
		...
		else if( r.type==Entry::CVAR_INT)	ofs<<names.str<<"="<<*(int*)r.data<<"\n";
	}
}
...
{	
	...
	for(names.it_start();names.it_running();names.it_next())
	{
		...
		{
			GetPrivateProfileString("CVAR",names.str,"NULL",cvar_f,size,getOgcDirFile("cvar.txt").c_str());
			*(float*)r.data = atof(cvar_f);
		}
		if( r.type==Entry::CVAR_INT )
		{
			GetPrivateProfileString("CVAR",names.str,"NULL",cvar_i,size,getOgcDirFile("cvar.txt").c_str());
			*(int*)r.data = atof(cvar_i);
		}
	}
}