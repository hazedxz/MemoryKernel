...
struct PrivateData{MapIntInt findByID; StringFinder findByName;};
...
{
	int index;
	if (currentSlot == -1)return -1;
	if (currentSlot == 0)index = slot1Index;
	else if (currentSlot == 1)index = slot2Index;
	else return -1;
	return index;
}
